...
class MyprestaComment extends ObjectModel
{
    ...
    public static function getByProduct($id_product, $p = 1, $n = null, $id_customer = null)
    {
        ...
        if (!Cache::isStored($cache_id)) {
			...
			WHERE pc.`id_product` = ' . (int)($id_product) . ($validate == '1' ? ' AND pc.`validate` = 1' : '') . (Configuration::get('myprestacomments_share_multi') != 1 ? ' AND pc.`id_shop` = ' . (int)Context::getContext()->shop->id : '') . '
    ...
    public static function getGradeByProduct($id_product, $id_lang)
    {
		...
		WHERE pc.`id_product` = ' . (int)$id_product . (Configuration::get('myprestacomments_share_multi') != 1 ? ' AND pc.`id_shop` = ' . (int)Context::getContext()->shop->id : '') . '
    ...
    public static function getCommentNumber($id_product)
    {
        ...
        if (!Cache::isStored($cache_id)) {
			...
			FROM `' . _DB_PREFIX_ . 'product_comment` pc
			WHERE `id_product` = ' . (int)($id_product) . ($validate == '1' ? ' AND `validate` = 1' : '') . (Configuration::get('myprestacomments_share_multi') != 1 ? ' AND pc.`id_shop` = ' . (int)Context::getContext()->shop->id : ''));
            Cache::store($cache_id, $result);
        }
        return Cache::retrieve($cache_id);
    }
    ...
    public static function getByValidate($validate = '0', $deleted = false)
    {
        $sql = '
			...
			WHERE pc.`validate` = ' . (int)$validate . (Configuration::get('myprestacomments_share_multi') != 1 ? ' AND pc.`id_shop` = ' . (int)Context::getContext()->shop->id : '');
    ...
    public static function getAllActivePro($id_product, $p = 1, $n = null)
    {
        ...
        $result = Db::getInstance(_PS_USE_SQL_SLAVE_)->executeS('
            ...
            ((int)$id_customer ? '(SELECT count(*) FROM `' . _DB_PREFIX_ . 'product_comment_usefulness` pcuc WHERE pcuc.`id_product_comment` = pc.`id_product_comment` AND pcuc.id_customer = ' . (int)$id_customer . ') as customer_advice, ' : '') .
            ((int)$id_customer ? '(SELECT count(*) FROM `' . _DB_PREFIX_ . 'product_comment_report` pcrc WHERE pcrc.`id_product_comment` = pc.`id_product_comment` AND pcrc.id_customer = ' . (int)$id_customer . ') as customer_report, ' : '') . '
			pc.`content`, pc.`grade`, pc.`date_add`, pc.title, 
			pc.`id_product_comment`, pc.`id_product`, 	IF(c.id_customer, CONCAT(c.`firstname`, \' \',  LEFT(c.`lastname`, 1)), pc.customer_name) customer_name, pc.`content`, pc.`grade`, pc.`date_add`, pl.`link_rewrite`, pl.`name`
			  FROM `' . _DB_PREFIX_ . 'product_comment` pc
			LEFT JOIN `' . _DB_PREFIX_ . 'customer` c ON c.`id_customer` = pc.`id_customer`
			INNER JOIN `' . _DB_PREFIX_ . 'product_lang` pl ON (pl.`id_product` = pc.`id_product` AND pl.`id_lang` = ' . (int)Context::getContext()->language->id . Shop::addSqlRestrictionOnLang('pl') . ')
			' . ((int)Configuration::get('PRODUCT_COMMENTS_MODERATE') == 1 ? ' WHERE  pc.`validate` = 1' : '') . (Configuration::get('myprestacomments_share_multi') != 1 ? ' AND pc.`id_shop` = ' . (int)Context::getContext()->shop->id : '') . '
			ORDER BY pc.`date_add` DESC
    ...
    public static function getAll()
    {
		...
		LEFT JOIN `' . _DB_PREFIX_ . 'customer` c ON (c.`id_customer` = pc.`id_customer`)
		LEFT JOIN `' . _DB_PREFIX_ . 'product_lang` pl ON (pl.`id_product` = pc.`id_product` AND pl.`id_lang` = ' . (int)Context::getContext()->language->id . Shop::addSqlRestrictionOnLang('pl') . ')
		' . (Configuration::get('myprestacomments_share_multi') != 1 ? ' AND pc.`id_shop` = ' . (int)Context::getContext()->shop->id : '') . '
		ORDER BY pc.`date_add` DESC'));
    }
