...
<div id="myprestacommentsBlock" {if $create_snippet}itemscope="" itemtype="http://schema.org/product"{/if}>
    {if $create_snippet}
    <meta itemprop="name" content="{$myprestacomments_product->name}">
    <meta itemprop="url" content="{Context::getContext()->link->getProductLink($myprestacomments_product)}">
    {/if}
        ...
        <div id="product_comments_block_tab">
            {if $comments}
                {foreach from=$comments item=comment}
                    {if $comment.content}
                        <div class="comment clearfix" {if $create_review_snippet}itemprop="review" itemscope itemtype="https://schema.org/Review"{/if}>
                            <div class="comment_author">
                                <span>{l s='Grade' mod='myprestacomments'}&nbsp</span>
                                <div class="star_content clearfix" {if $create_review_snippet} itemprop="reviewRating" itemscope itemtype="https://schema.org/Rating" {/if}>
                                ...
                                <div class="comment_author_infos">
                                    {if $PRODUCT_COMMENT_AUTHOR == 1}
                                        <span {if $create_review_snippet}itemprop="author" itemscope itemtype="https://schema.org/Person"{/if}>
                                        <strong {if $create_review_snippet}itemprop="name"{/if}>{$comment.customer_name|escape:'html':'UTF-8'}</strong><br/>
                                        </span>
                                    {/if}
                                    {if $PRODUCT_COMMENT_DATE == 1}
                                        <em>{dateFormat date=$comment.date_add|escape:'html':'UTF-8' full=0}</em>
                                    {/if}
                                    <meta {if $create_review_snippet}itemprop="datePublished"{/if} content="{dateFormat date=$comment.date_add|escape:'html':'UTF-8' full=0}"/>
                                </div>
                            </div>
                            <div class="comment_details">
                                {if $PRODUCT_COMMENT_TITLE == 1}
                                    <h4 class="title_block" {if $create_review_snippet}itemprop="name"{/if}>{$comment.title}</h4>
                                {/if}
                                {if $PRODUCT_COMMENT_BODY == 1}
                                    <p {if $create_review_snippet}itemprop="reviewBody"{/if}>{$comment.content|escape:'html':'UTF-8'|nl2br nofilter}</p>
                                {/if}
                    ...
                    <div class="new_comment_form_content">
                        ...
                        <div id="new_comment_form_footer">
                            ...
                            <p class="fr">
                                ...
                                {/literal}
                                    {l s='I accept ' mod='myprestacomments'}
                                    <a target="_blank" href="{$link->getCmsLink($PRODUCT_COMMENTS_GDPRCMS)}">{l s='privacy policy' mod='myprestacomments'}</a>
                                    {l s='rules' mod='myprestacomments'}
                                {/if}
