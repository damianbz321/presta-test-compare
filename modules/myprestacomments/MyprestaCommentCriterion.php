...
class MyprestaCommentCriterion extends ObjectModel
{
    public $id;
    public $id_shop;
     ...
     */
    public static function getByProductAndByComment($id_comment)
    {
        $id_lang = Context::getContext()->language->id;
        $table = '_shop';
        $alias = 'ps';

        $cache_id = 'MyprestaCommentCriterion::getByProductAndByComment_' . (int)$id_comment . '-' . (int)$id_lang;
        if (!Cache::isStored($cache_id)) {
            $result = Db::getInstance()->executeS('
				SELECT pcg.`id_product_comment_criterion`, pccl.`name`, pcg.`grade`
				FROM `' . _DB_PREFIX_ . 'product_comment` pc
				LEFT JOIN `' . _DB_PREFIX_ . 'product_comment_grade` pcg
				    ON (pc.id_product_comment = pcg.id_product_comment)
				LEFT JOIN `' . _DB_PREFIX_ . 'product_comment_criterion_lang` pccl
					ON (pcg.id_product_comment_criterion = pccl.id_product_comment_criterion)
				
				WHERE pccl.`id_lang` = ' . (int)($id_lang) . (Configuration::get('myprestacomments_share_multi') != 1 ? ' AND pc.`id_shop` = ' . (int)Context::getContext()->shop->id : '') . '
				AND pc.`id_product_comment` = ' . $id_comment . '
			');
    ...
    public static function getByProduct($id_product, $id_lang)
    {
        ...
        $cache_id = 'MyprestaCommentCriterion::getByProduct_' . (int)$id_product . '-' . (int)$id_lang;
        if (!Cache::isStored($cache_id)) {
            $result = Db::getInstance()->executeS('
				...
				WHERE pccl.`id_lang` = ' . (int)($id_lang) . (Configuration::get('myprestacomments_share_multi') != 1 ? ' AND pcc.`id_shop` = ' . (int)Context::getContext()->shop->id:''). '
				AND (
    ...
    public static function getCriterions($id_lang, $type = false, $active = false)
    {
        ...
        $sql = '
			...
			WHERE pccl.`id_lang` = ' . (int)$id_lang . ($active ? ' AND active = 1' : '') . ($type ? ' AND id_product_comment_criterion_type = ' . (int)$type : '') .(Configuration::get('myprestacomments_share_multi') != 1 ? ' AND pcc.`id_shop` = ' . (int)Context::getContext()->shop->id:''). '
    ...
    public static function getTypes()
    {
        ...
        $module = new myprestacomments();

        return array(
            1 => $module->l('Valid for the entire catalog', 'MyprestaCommentCriterion'),
            2 => $module->l('Restricted to some categories', 'MyprestaCommentCriterion'),
            3 => $module->l('Restricted to some products', 'MyprestaCommentCriterion')
        );
    }
}
