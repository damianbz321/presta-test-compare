...
class DataFeedWatch extends Module
{
    ...
    protected function prepareProductAttributeData($attributeProductId, $item, $productData, $shopId, $langId)
    {
        ...
        $attributesProductData = $product->getAttributeCombinationsById(
            $attributeProductId,
            $langId
        );
        $attributeProductData = array();
        if (count($attributesProductData) > 0) {
            foreach ($attributesProductData as $attributeItem) {
                if ((int)$shopId === (int)$attributeItem['id_shop']) {
                    $attributeProductData = $attributeItem;
                }
            }
        }
        ...
        $productData['minimalQuantity'] = (int)$attributeProductData['minimal_quantity'];
        ...
        $productData['deliveryDate'] = $this->prepareProductCombinationDeliveryData(
            $attributeProductData,
            $productData['deliveryDate']
        );
        
        $productData['location'] = StockAvailable::getLocation((int) $productId, (int) $attributeProductId, $shopId);
    ...
    protected function prepareProductCombinationDeliveryData($attributeProductData, $deliveryDate)
    {
        if (!empty($attributeProductData['available_date'])
            && '0000-00-00' !== $attributeProductData['available_date']
        ) {
            $now = time();
            $availableDate = strtotime($attributeProductData['available_date']);
            $dateDiff =  $availableDate - $now;
            $dateDiff = round($dateDiff / (60 * 60 * 24));

            return $dateDiff > 0 ? $dateDiff : $deliveryDate;
        }
    ...
    protected function fetchProductList($params, $shopId)
    {
        ...
        $dbquery->join('LEFT JOIN `' . _DB_PREFIX_ . 'stock_available`'
            . ' stock ON stock.`id_product` = p.`id_product` AND stock.`id_shop` = ' . (int)$shopId . ' AND stock.`id_product_attribute` = 0');
