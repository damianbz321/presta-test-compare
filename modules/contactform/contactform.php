...
class Contactform extends Module implements WidgetInterface
{
    ...
    public function sendMessage()
    {
        ...
        if(!isset($_REQUEST['_token']) || $_REQUEST['_token'] != $serverToken){
        ...
        if (strpos(Tools::getValue('from'), 'godaddy') !== false) {
            $this->context->controller->errors[] = $this->trans(
                'An error occurred while sending the message, please try again.',
                [],
                'Modules.Contactform.Shop'
        ...
        if (!($from = trim(Tools::getValue('from'))) || !Validate::isEmail($from)) {
            ...
            // check email message spam
            $spamBool = $this->checkSpamMessage($message);
    ...
    private function checkSpamMessage($msg) {
        $spamStrings = [
            'হাই','Hallo,','Hola,','Ciao,','Ndewo,', 'Sveiki,','Hi, ego','Hej, jeg','Hi, I wanted','Γεια σου,',
            'Hi, kam','Hi, roeddwn','Salam,','Ողջույն,','Dia duit,','Hai,','Hi, მინდოდა','Ola, quería','হাই,',
            'Здравейте,','Aloha,','Zdravo,','Прывітанне,','Szia, meg','Sawubona,','Kaixo,','Salut, ech','Xin chào,','Hæ,','Hello Studiomocy','imagine this…'
        ];
