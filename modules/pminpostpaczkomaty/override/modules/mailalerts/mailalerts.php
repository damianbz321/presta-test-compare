    ...
    public function hookActionValidateOrder($params)
    {
        if (Module::isInstalled('pminpostpaczkomaty') && Module::isEnabled('pminpostpaczkomaty')) {
            ...
            foreach ($products as $key => $product) {
                $unit_price = Product::getTaxCalculationMethod($customer->id) == PS_TAX_EXC ? $product['product_price']
                : $product['product_price_wt'];

                $customization_text = '';
                if (isset($customized_datas[$product['product_id']][$product['product_attribute_id']])) {
                    foreach ($customized_datas[
                            $product['product_id']
                        ][$product['product_attribute_id']][$order->id_address_delivery] as $customization) {
            ...
            foreach ($params['order']->getCartRules() as $discount) {
                ...
                '<tr style="background-color:#EBECEE;">
                            ...
                            $this->l('Voucher code:').' '.$discount['name'].'</td>
                        <td style="padding:0.6em 0.4em; text-align:right;">-'.
                        Tools::displayPrice(
                            $discount['value'],
                            $currency,
            ...
            foreach ($merchant_mails as $merchant_mail) {
                ...
                $dir_mail = false;
                if (file_exists(dirname(__FILE__).'/../../../modules/mailalerts/mails/'.$mail_iso.'/new_order.txt') &&
                    file_exists(dirname(__FILE__).'/../../../modules/mailalerts/mails/'.$mail_iso.'/new_order.html')) {
                    $dir_mail = dirname(__FILE__).'/../../../modules/mailalerts/mails/';
                }
