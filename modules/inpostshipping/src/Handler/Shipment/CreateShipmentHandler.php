...
class CreateShipmentHandler
{
    ...
    protected function saveShipment(Order $order, Shipment $shipment)
    {
        ...
        $shipmentModel->reference = in_array($shipment->service, Service::LOCKER_SERVICES)
            ? $shipment->reference
            : $shipment->comments;
        $shipmentModel->email = $shipment->receiver['email'];
        $shipmentModel->phone = $shipment->receiver['phone'];
        $shipmentModel->service = $shipment->service;
        ...
        if ($shipment->service === Service::INPOST_LOCKER_STANDARD) {
