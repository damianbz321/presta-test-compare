...
abstract class AdminInPostShipmentsController extends InPostShippingAdminController
{
    ...
    public function processBulkPrintLabels()
    {
        if (is_array($this->boxes) && !empty($this->boxes)) {
            $shipmentIds = InPostShipmentModel::getShipXShipmentIds($this->boxes, $this->sandbox);

            $labels = Shipment::getMultipleLabels(
                $shipmentIds,
                $this->resolvePrintOptions(true)
            );

            InPostShipmentModel::setLabelPrinted($shipmentIds);
    ...
    protected function resolvePrintOptions($withFormat = false)
    {
        $query = [];
        ...
        if ($withFormat) {
            $query['format'] = in_array($format = Tools::getValue('label_format'), Shipment::LABEL_TYPES)
                ? $format
                : Shipment::FORMAT_PDF;
        }
