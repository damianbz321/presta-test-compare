...
class LabCategoryClass extends ObjectModel
{
	public $id_labgroupcategory;
	public $id_labcategory;
	public $id_cat;
	public $cat_color;
	public $cat_desc;
	public $cat_banner;
	...
	public static $definition = array(
		'table' => 'labcategory',
		'primary' => 'id_labcategory',
		'multilang' => true,
		'multilang_shop' => true,
		'fields' => array(
			'id_labgroupcategory' =>	array('type' => self::TYPE_INT, 'shop' => true, 'validate' => 'isunsignedInt', 'required' => true),
	...
	public	function __construct($id_labcategory = null, $id_lang = null, $id_shop = null, Context $context = null)
	{
		parent::__construct($id_labcategory, $id_lang, $id_shop);
		Shop::addTableAssociation('labcategory', array('type' => 'shop'));
		Shop::addTableAssociation('labcategory_lang', array('type' => 'fk_shop'));
	...
	public function getFieldShop($field)
	{
		...
		$sql = 'SELECT wms.'.$field.' FROM '._DB_PREFIX_.'labcategory wm
		LEFT JOIN '._DB_PREFIX_.'labcategory_shop wms ON (wm.id_labcategory = wms.id_labcategory)
		WHERE wm.id_labcategory = '.$this->id.' AND wms.id_shop = '.$id_shop.'';
	...
	public function delete()
	{
		...
		foreach ($cat_banner as $image)
		{
			if (preg_match('/sample/', $image) === 0)
				if ($image && file_exists(_PS_MODULE_DIR_.'labproductcategory/views/img/banners/'.$image))
					$res &= @unlink(_PS_MODULE_DIR_.'labproductcategory/views/img/banners/'.$image);
		}
		
		if (preg_match('/sample/', $cat_icon) === 0)
			if ($cat_icon && file_exists(_PS_MODULE_DIR_.'labproductcategory/views/img/icons/'.$cat_icon))
				$res &= @unlink(_PS_MODULE_DIR_.'labproductcategory/views/img/icons/'.$cat_icon);
	...
	public function deleteIcon()
	{
		...
		if (preg_match('/sample/', $cat_icon) === 0)
			if ($cat_icon && file_exists(_PS_MODULE_DIR_.'labproductcategory/views/img/icons/'.$cat_icon))
				$res &= @unlink(_PS_MODULE_DIR_.'labproductcategory/views/img/icons/'.$cat_icon);
		$sql = 'UPDATE '._DB_PREFIX_.'labcategory_shop SET cat_icon = "'.$cat_icon_new.'" WHERE id_labcategory = '.$this->id.' AND id_shop = '.$id_shop;
		$sql1 = 'UPDATE '._DB_PREFIX_.'labcategory SET cat_icon = "'.$cat_icon_new.'" WHERE id_labcategory = '.$this->id;
	...
	public function deleteBanner()
	{
		...
		foreach ($cat_banner as $key => $image)
		{
			if (preg_match('/sample/', $image) === 0)
				if ($image && file_exists(_PS_MODULE_DIR_.'labproductcategory/views/img/banners/'.$image))
					$res &= @unlink(_PS_MODULE_DIR_.'labproductcategory/views/img/banners/'.$image);
			$sql = 'UPDATE '._DB_PREFIX_.'labcategory_lang SET cat_banner = "'.$banner_new.'" WHERE id_labcategory = '.$this->id.' AND id_lang = '.$key.' AND id_shop = '.$id_shop;
			$res &= Db::getInstance(_PS_USE_SQL_SLAVE_)->execute($sql);
		}
		return $res;
	}	
	public function getCatByGroupId($id_group)
	{
		...
		return Db::getInstance(_PS_USE_SQL_SLAVE_)->executeS('
			...
			LEFT JOIN '._DB_PREFIX_.'labcategory_lang wl ON (wc.`id_labcategory` = wl.`id_labcategory` AND wc.id_shop = wl.id_shop)
			WHERE wc.id_labgroupcategory = '.$id_group.' AND wl.id_shop = '.$id_shop.'
			AND wl.id_lang = '.$id_lang.' AND wc.active = 1');
	}
	public  function getItembyCatId($idCat) {
		...
		return Db::getInstance(_PS_USE_SQL_SLAVE_)->executeS('
			...
			FROM '._DB_PREFIX_.'labcategory_shop wc
			LEFT JOIN '._DB_PREFIX_.'labcategory_lang wl ON (wc.`id_labcategory` = wl.`id_labcategory` AND wc.id_shop = wl.id_shop)
			WHERE wc.id_cat = '.$idCat.' AND wl.id_shop = '.$id_shop.'
			AND wl.id_lang = '.$id_lang.' AND wc.active = 1');
