...
<div class="lab_category_feature laberthemes">
		...
		{if isset($lab_categories) && $lab_categories|@count > 0}
							...
							<div class="item">
								...
								<div class="cat-img">
									<a href="{$link->getCategoryLink($category->id_category, $category->link_rewrite)|escape:'html':'UTF-8'}" title="{$category->name|escape:'html':'UTF-8'}">
										...
										<img  class="replace-2x" alt="{$category->name|escape:'html':'UTF-8'}" src="{$lab_cat_img}/img/c/{$category->id_category}.jpg"/>
									</a>
								</div>
								{/if}
			...
			<div class="lab_boxnp">
				...
				<a class="next nextcategory"><i class="icon icon-angle-right"></i></a>
			</div>
			{/if}
		{else}
