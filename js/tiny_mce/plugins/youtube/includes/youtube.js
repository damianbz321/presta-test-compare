jQuery(document).ready(function ($) {
    ...
    $("#queryinput").autocomplete({
        source: function (request, response) {
            var apiKey = 'AIzaSyD2a_o9RNvubMXm8W-iwlsrTPLn2BEVj1g';
            if (apiKey == '')
            {
                alert("Insert apiKey in youtube.js");
            }
    ...
    // Adjust button text based on click state
    $("#youtube_hide").click(function () {
        isset_rel = $(this).is(':checked');
        if (isset_rel == true) {
            $('#youtube_hide_label > span').html('On');
        }
        else {
            $('#youtube_hide_label > span').html('Off');
        }
    });

    // Adjust button text based on click state
    $("#youtube_ctrl").click(function () {
        isset_rel = $(this).is(':checked');
        if (isset_rel == true) {
            $('#youtube_ctrl_label > span').html('On');
        }
        else {
            $('#youtube_ctrl_label > span').html('Off');
        }
    });
