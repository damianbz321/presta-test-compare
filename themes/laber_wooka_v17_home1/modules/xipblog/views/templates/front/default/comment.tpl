<div class="comment_respond clearfix m_bottom_50" id="respond">
    ...
    <form class="comment_form" action="" method="post" id="xipblogs_commentfrom" role="form" data-toggle="validator">
    	...
    	<div class="form-group xipblog_name_parent">
    	  <label for="xipblog_name">Twoje imię:</label>
    	...
    	<div class="form-group xipblog_email_parent">
    	  <label for="xipblog_email">Twój email:</label>
    	  <input type="email"  id="xipblog_email" name="xipblog_email" class="form-control xipblog_email" required>
    	</div>
    	<div class="form-group xipblog_website_parent" style="display: none;">
    	  <label for="xipblog_website">Adres strony internetowej:</label>
    	  <input type="url"  id="xipblog_website" name="xipblog_website" class="form-control xipblog_website">
    	</div>
    	<div class="form-group xipblog_subject_parent" style="display: none;">
    	  <label for="xipblog_subject">Temat:</label>
    	  <input type="text"  value="HiFood comment" id="xipblog_subject" name="xipblog_subject" class="form-control xipblog_subject" required>
    	</div>
    	...
    	<input type="submit" class="btn btn-default pull-left xipblog_submit_btn" value="Wyślij">
    </form>
</div>
...
$('.xipblog_submit_btn').on("click",function(e) {
	...
	if(!$(this).hasClass("disabled")){
		...
		$.ajax({
			...
			complete: function(){
				$(".xipblog_submit_btn").val("Wyślij");
				$(".xipblog_submit_btn").addClass("disabled");
			},
			success: function(data){
				$('.xipblog_submit_btn').prop('disabled', false);
				$('#xipblogs_commentfrom').append('<div style="clear: both;margin-top: 70px;" class="alert alert-success">Komentarz został dodany, za chwile nastąpi odświeżenie strony.</div>');
				setTimeout(function() {
					location.reload();
				}, 3000);
