...
<div class="laberthemes padding-0-10">
	<div class="laberColumn labProductList">
		{foreach from=$group_cat_result item=group_cat name=group_cat_result}
			...
			{foreach from=$group_cat_info item=cat_info name=g_cat_info}
				 ...
				 <div class="block-content">
					<div class="laberProdCategory labProdCat-{$cat_info.id_cat|intval}">
						<div class="title_block">
							...
							<h3>
								<a href="{$link->getCategoryLink($cat_info.id_cat, $cat_info.link_rewrite)|escape:'html':'UTF-8'}" title="{$cat_info.cat_name|escape:'html':'UTF-8'}">
									{$cat_info.cat_name|escape} <span>{l s='Products' d='Shop.Theme.Laberthemes'}</span>
								</a>
							</h3>
						  
						
						{if $show_sub}
							<span class="sub_category sub_category-{$cat_info.id_cat|intval} ">{l s='Sub Category' d='Shop.Theme.Laberthemes'} <i class="fa fa-angle-down" aria-hidden="true"></i></span>
							<ul class="tab_cates">
								{foreach from = $cat_info.sub_cat item=sub_cat name=sub_cat_info}
									{if $smarty.foreach.sub_cat_info.iteration <=4}
						...
						<div class="laberrow {if $id%2 == 0}laberright{/if} ">
							{if $cat_info.cat_banner!='' }
							<div class="laberCat-banner left-colulm">
						...
						{if $group_cat.use_slider == 1}
							<div class="owl-buttons">
								<div class="owl-prev prev-{$id}{$cat_info.id_cat|intval}"><i class="icon-chevron-left icon"></i></div>
								<div class="owl-next next-{$id}{$cat_info.id_cat|intval}"><i class="icon-chevron-right icon"></i></div>
							</div>
						{/if}
						{if $group_cat.use_slider == 1}
							<script type="text/javascript">
								$(document).ready(function() {
									...
									$('.laberthemes .title_block .sub_category-{$cat_info.id_cat|intval}').click(function() {
