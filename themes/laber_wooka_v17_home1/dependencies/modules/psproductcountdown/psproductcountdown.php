...
class PSProductCountdown extends Module
{
    ...
    public function install()
    {
		...
		CREATE TABLE IF NOT EXISTS `' . _DB_PREFIX_ . 'psproductcountdown` (
        ...
        // lang table
        $db_result = Db::getInstance()->execute('
		CREATE TABLE IF NOT EXISTS `' . _DB_PREFIX_ . 'psproductcountdown_lang` (
        ...
        foreach ($this->getSettings() as $item) {
            ...
            if (Configuration::get($this->settings_prefix . $item['name']) === false) {
                if (isset($item['default_'.$this->getPSVersion(true)])) {
                    Configuration::updateValue($this->settings_prefix . $item['name'], $item['default_'.$this->getPSVersion(true)]);
    ...
    public function hookHeader()
    {
        ...
        $this->context->controller->addJS($this->_path . 'views/js/psproductcountdown.js');
        $this->context->controller->addCSS(($this->_path) . 'views/css/psproductcountdown.css');
        if ($this->theme) {
    ...
    public function hookActionProductUpdate($params)
    {
        ...
        if (Tools::isSubmit($this->name . '-submit')) {
            ...
            $id_countdown = Db::getInstance()->getValue(
                'SELECT `id_countdown` 
                FROM `' . _DB_PREFIX_ . 'psproductcountdown` 
            ...
            if ($id_countdown) {
                ...
                if (!$from && !$to && $active) {
                    Db::getInstance()->execute(
                        'DELETE FROM `' . _DB_PREFIX_ . 'psproductcountdown`
                        WHERE `id_countdown` = ' . (int)$id_countdown
                    );
                } else {
                    // Save
                    Db::getInstance()->execute(
                        'UPDATE `' . _DB_PREFIX_ . 'psproductcountdown`
                    ...
                    foreach (Language::getLanguages(true, false, true) as $id_language) {
                        ...
                        $name = Tools::getValue('pspc_name_' . $id_language);
                        Db::getInstance()->execute(
                            'UPDATE `' . _DB_PREFIX_ . 'psproductcountdown_lang`
                            SET `name` = "' . pSQL($name) . '"
                ...
                if (!$from && !$to && $active) {
                    ...
                    // add countdown
                    Db::getInstance()->execute(
                        'INSERT INTO `' . _DB_PREFIX_ . 'psproductcountdown`
                    ...
                    foreach (Language::getLanguages(true, false, true) as $id_language) {
                        ...
                        $name = Tools::getValue('pspc_name_' . $id_language);
                        Db::getInstance()->execute(
                            'INSERT INTO `' . _DB_PREFIX_ . 'psproductcountdown_lang`
    ...
    public function getContent()
    {
        ...
        $this->html .= $this->renderAdditionalInstructions();
    ...
    public function getSettings()
    {
        $settings = array(
            ...
            array(
                ...
                'hint' => $this->l('Enable auto-clearing Smarty cache after saving settings. Also you can do it manually at the Performance tab as usually.'),
                'default' => 1,
                'validate' => 'isInt',
            ),
            array(
                ...
                'hint' => $this->l('Enable auto-clearing Smarty cache after saving products. Also you can do it manually at the Performance tab as usually.'),
                'default' => 1,
                'validate' => 'isInt',
            ),
            array(
    ...
    public function getCountdownBOData($id_product)
    {
        $countdown = Db::getInstance()->getRow(
            'SELECT *
            FROM `' . _DB_PREFIX_ . 'psproductcountdown` pspc
        ...
        foreach (Language::getLanguages(true, false, true) as $id_lang) {
            ...
            $names[$id_lang] = Db::getInstance()->getValue(
                'SELECT `name`
                FROM `' . _DB_PREFIX_ . 'psproductcountdown_lang` pspcl
    ...
    public function getCountdown($id_product)
    {
        $countdown = Db::getInstance()->getRow(
            'SELECT *
            FROM `' . _DB_PREFIX_ . 'psproductcountdown` pspc
            LEFT JOIN  `' . _DB_PREFIX_ . 'psproductcountdown_lang` pspcl ON pspc.`id_countdown` = pspcl.`id_countdown`
    ...
    public function getCountdownsList()
    {
        $active_countdowns = Db::getInstance()->executeS(
            'SELECT pspc.*, pspcl.*, pl.`name` AS `product_name`, 0 AS `expired`
            FROM `' . _DB_PREFIX_ . 'psproductcountdown` pspc
            LEFT JOIN  `' . _DB_PREFIX_ . 'psproductcountdown_lang` pspcl ON pspc.`id_countdown` = pspcl.`id_countdown`
            LEFT JOIN  `' . _DB_PREFIX_ . 'product_lang` pl ON pspc.`id_product` = pl.`id_product` AND pspcl.`id_lang` = pl.`id_lang` AND pspc.`id_shop` = pl.`id_shop`
            WHERE pspc.`id_shop` = ' . (int)$this->context->shop->id . '  AND pspcl.`id_lang` = ' . (int)$this->context->language->id .'
            AND pspc.`from` <= NOW() AND pspc.`to` >= NOW()
            ORDER BY pspc.`id_product`'
        );

        $inactive_countdowns = Db::getInstance()->executeS(
            'SELECT pspc.*, pspcl.*, pl.`name` AS `product_name`, 1 AS `expired`
            FROM `' . _DB_PREFIX_ . 'psproductcountdown` pspc
            LEFT JOIN  `' . _DB_PREFIX_ . 'psproductcountdown_lang` pspcl ON pspc.`id_countdown` = pspcl.`id_countdown`
            LEFT JOIN  `' . _DB_PREFIX_ . 'product_lang` pl ON pspc.`id_product` = pl.`id_product` AND pspcl.`id_lang` = pl.`id_lang` AND pspc.`id_shop` = pl.`id_shop`
            WHERE pspc.`id_shop` = ' . (int)$this->context->shop->id . '  AND pspcl.`id_lang` = ' . (int)$this->context->language->id .'
            AND (pspc.`from` > NOW() OR pspc.`to` < NOW())
            ORDER BY pspc.`id_product`'
        );
    ...
    public function ajaxProcessRemoveCountdown()
    {
        ...
        if ($id_countdown) {
            Db::getInstance()->execute('DELETE FROM `' . _DB_PREFIX_ . 'psproductcountdown` WHERE `id_countdown` = '.(int)$id_countdown);
            die('1');
        }
    }
    ...
    protected function clearSmartyCache()
    {
        if (method_exists($this, '_clearCache')) {
            ...
            $this->_clearCache('psproductcountdown');
        }
    }
